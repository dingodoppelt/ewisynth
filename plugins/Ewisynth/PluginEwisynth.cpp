...
*/
void PluginEwisynth::sampleRateChanged ( double newSampleRate ) { fSampleRate = newSampleRate; }

/**
  Get the current value of a parameter.
*/
float PluginEwisynth::getParameterValue ( uint32_t index ) const { return fParams[index]; }
...
void PluginEwisynth::setParameterValue ( uint32_t index, float value )
{
    ...
    case CONTROL_FILT_CUTOFF:
        if ( filterL != nullptr )
            filterL->SetCutoff ( getCurve ( value, MAX_FILTER_CUTOFF, getParameterValue ( CONTROL_FILT_CURVE ), true ) );
        if ( filterR != nullptr )
            filterR->SetCutoff ( getCurve ( value, MAX_FILTER_CUTOFF, getParameterValue ( CONTROL_FILT_CURVE ), true ) );
        break;
    case CONTROL_FILT_RESO:
        if ( filterL != nullptr )
            filterL->SetResonance ( value );
        if ( filterR != nullptr )
            filterR->SetResonance ( value );
        break;
    default:
        break;
    }
...
void PluginEwisynth::run ( const float** inputs, float** outputs, uint32_t frames, const MidiEvent* midiEvents, uint32_t midiEventCount )
{
        ...
        if ( midiEvents[i].size <= 3 )
        {
            ...
            for ( uint32_t j = offset; j <= midiEvents[i].frame; j++ )
            {
                if ( getParameterValue ( CONTROL_USE_RMS ) )
                {
                    ...
                    filterL->SetCutoff (
                        ( getCurve ( rms, 1.f - normalizedCutoff, getParameterValue ( CONTROL_FILT_CURVE ), true ) + normalizedCutoff ) *
                        (float) MAX_FILTER_CUTOFF );
                    filterR->SetCutoff (
                        ( getCurve ( rms, 1.f - normalizedCutoff, getParameterValue ( CONTROL_FILT_CURVE ), true ) + normalizedCutoff ) *
                        (float) MAX_FILTER_CUTOFF );
                }
            ...
            case 0x90:
                ...
                break;
            case 0xE0:
                polyfotz.setPitchbend ( midiEvents[i].data[2] << 7 |
                                        midiEvents[i].data[1] ); // 2^( ((pitchbend - 8192) / 8192 * bendrange = 2 / max_pitchbend = 16383) / 12 )
                break;
            default:
                break;
            }
    ...
    for ( uint32_t j = offset; j < frames; j++ )
    {
        if ( (bool) getParameterValue ( CONTROL_USE_RMS ) )
        {
            ...
            currPressure    = getCurve ( rms, 1.f, getParameterValue ( CONTROL_CURVE ), false );
            currPulseWidth  = getCurve ( rms, 1.f, getParameterValue ( CONTROL_PW_CURVE ), false ) / 2.f + .5f; // limit pulse width to .5 - 1.
            filterL->SetCutoff ( ( getCurve ( rms, 1.f - normalizedCutoff, getParameterValue ( CONTROL_FILT_CURVE ), true ) + normalizedCutoff ) *
                                 (float) MAX_FILTER_CUTOFF );
            filterR->SetCutoff ( ( getCurve ( rms, 1.f - normalizedCutoff, getParameterValue ( CONTROL_FILT_CURVE ), true ) + normalizedCutoff ) *
                                 (float) MAX_FILTER_CUTOFF );
...
Plugin* createPlugin() { return new PluginEwisynth(); }
